...
type GetOrder struct {
	...
	PrintFile string `xml:"print_file,omitempty"`

	// 是否需要打印文件的 json 数据，用于 直接生成打印图片:1 需要，0 不需要
	JSONData string `xml:"json_data,omitempty"`

	// 强制加密参数，1 加密
	JSONEncrypt string `xml:"json_encrypt,omitempty"`
}
...
type GetResponsesBody struct {
	...
	Response GetResponseBody `xml:"response"`
}

type GetResponseBody struct {
	Status      string `xml:"status"`
	Msg         string `xml:"msg"`
	GetResponse `xml:"innerxml"`
}
